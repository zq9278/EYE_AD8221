...
- UART (VOFA+): 1,000,000 bps, TX=P0.06, RX=P0.12, 3 channels
  - CH1: raw ADC (2 kHz)
  - CH2: envelope/energy (2 kHz)
  - CH3: notch-filtered (50/60 Hz) sample (2 kHz)
...
5) **Publish (full-rate) + decimation for BLE**  
   - `latest_sample_u16` (envelope/filtered) and `latest_notch_u16` (notched) updated every ADC sample (2 kHz) for UART.  
   - Separately, every `fs/out_hz` sample (`CONFIG_EYE_BLE_STREAM_HZ`, current 200 Hz) is enqueued into `stream_q` with incrementing `seq` for BLE.
...
- Notch: `CONFIG_EYE_NOTCH_ENABLED=y`, `CONFIG_EYE_NOTCH_FREQ_HZ=50`, `CONFIG_EYE_NOTCH_Q=25`, `CONFIG_EYE_NOTCH_DOUBLE=n`
